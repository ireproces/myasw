...
public class ApplicationContext {
	...
	// private static final String DEFAULT_SERVER_ADDRESS = "10.11.1.121";
    ...
    public synchronized Service getService(String serverHost) {
    	...
    	try {
			...
			// la classe InetAddress rappresenta un indirizzo IP e serve per
			// incapsulare l'associazione tra nome host ed indirizzo IP
    		InetAddress address = InetAddress.getByName(serverHost);
    		int port = SERVER_PORT;

			// crea un proxy lato client per il servizio offerto dal server
    		proxy = new ServiceClientProxy(address, port);
    ...
    public Client getClient(String serverHost) {
		...
		// inietta la dipendenza del client dal servizio
		// mascherato dal proxy lato client
