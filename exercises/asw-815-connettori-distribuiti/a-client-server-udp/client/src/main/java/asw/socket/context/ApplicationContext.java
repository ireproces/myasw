...
public class ApplicationContext {
	...
	// metodo invocato da getClient(String serverHost)
	// ritorna il proxy lato client (configurato) del servizio
	...
	// metodo invocato senza parametri, a sua volta invoca quello con parametri
	// fornendo l'indirizzo di default del server (localhost)
    public Client getClient() {
		return getClient(SERVER_ADDRESS);
    }

	// metodo invocato con parametri:
	// - dal main, quando viene specificato l'indirizzo da linea di comando
	// - dal metodo precedente, quando non viene specificato da linea di comando
    public Client getClient(String serverHost) {
		logger.info("ApplicationContext: getClient(" + serverHost + ")");
		Client client = new Client();

		// il client usa il servizio tramite dipendenza dal proxy
